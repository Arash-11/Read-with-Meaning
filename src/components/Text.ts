...
export default class Text {
  ...
  get selectedWord(): string {
    ...
    /*
      Limit `endOffset` to additional 50 boundary points in the range, to avoid grabbing
      entire large batches of texts unnecessarily, as that can cause performance issues
      with large amounts of text (eg. a 50-page PDF).
    */
    ...
    if (startOffset === 0) {
      ...
      */
      ...
      /*
        ...
        iterating backwards, as defined in the for-loop above, or it'll remain an empty string.
      */
      if (selection.toString()) word = selection.toString();
      else if (rangeStartText) word = rangeStartText.match(/(\w+)/g)![0];
